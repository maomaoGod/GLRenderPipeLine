...
int main()
{
    ...
    Shader shader("../Resources/Shaders/5.2.framebuffers.vs", "../Resources/Shaders/5.2.framebuffers.fs");
    Shader screenShader("../Resources/Shaders/depth_screen.vs", "../Resources/Shaders/depth_screen.fs");
    Shader depthShader("../Resources/Shaders/depth_view.vs","../Resources/Shaders/depth_view.fs");
    Shader stencilShader("../Resources/Shaders/2.stencil_testing.vs","../Resources/Shaders/2.stencil_single_color.fs");
    Shader skyboxShader((shaderpathprefix+"skybox.vs").c_str(), (shaderpathprefix+"skybox.fs").c_str());
    ...
    float depthQuadVertices[] = { // vertex attributes for a quad that fills the entire screen in Normalized Device Coordinates. NOTE that this plane is now much smaller and at the top of the screen
            // positions   // texCoords
            0.2f,  0.3f,  0.0f, 1.0f,
            0.2f,  -0.3f,  0.0f, 0.0f,
            1.0f,  -0.3f,  1.0f, 0.0f,
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(cubVerticesWithNorm), &cubVerticesWithNorm, GL_STATIC_DRAW);
    ...
    unsigned int depthquadVAO, depthquadVBO;
    glGenVertexArrays(1, &depthquadVAO);
    glGenBuffers(1, &depthquadVBO);
    glBindVertexArray(depthquadVAO);
    glBindBuffer(GL_ARRAY_BUFFER, depthquadVBO);
    ...
    // -------------------------
    //全景小窗口
    unsigned int allviewframebuffer=0,allviewtextureColorbuffer=0, allviewrenderbufferobj =0;
    BuildFrameBuffer(allviewframebuffer,allviewtextureColorbuffer,allviewrenderbufferobj);
    //深度图小窗口
    unsigned int depthframebuffer=0,depthtextureColorbuffer=0, depthrenderbufferobj =0;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // ------------------------------------------------------------------------
        glBindFramebuffer(GL_FRAMEBUFFER, allviewframebuffer);
        ...
        if(SCR_WIDTH == window_width && SCR_HEIGHT == window_height){
        ...
        //绘制爆炸效果
        model = glm::mat4(1.0f);
        suitexplodeShader.use();
        suitexplodeShader.setMat4("projection", projection);
        suitexplodeShader.setMat4("view", view);
        model = glm::scale(model,glm::vec3(0.1f,0.1f,0.1f));
        suitexplodeShader.setMat4("model", model);
        // add time component to geometry shader in the form of a uniform
        suitexplodeShader.setFloat("time", glfwGetTime());
        // draw model
        ...
        suitnormalvisualShader.use();
        suitnormalvisualShader.setMat4("projection",projection);
        suitnormalvisualShader.setMat4("view",view);
        suitnormalvisualShader.setMat4("model",model);
        suitnormalvisualShader.setFloat("MAGNITUDE",0.004);
        suitmodel.Draw(suitnormalvisualShader);
        ...
        shader.setMat4("projection",projection);
        glBindVertexArray(grassVAO);
        std::map<float, std::map<int,glm::vec3>> sorted;
        for(unsigned int i = 0; i < grasspos.size(); i++)
        {
            float dist = glm::length(camera.Position - grasspos[i]);
            std::map<int,glm::vec3> transit;
            transit[1] = grasspos[i];
            sorted[dist] = transit;
        }

        for(unsigned int i=0;i<grasspos.size();i++){
            glm::vec3 winpos = grasspos[i]+glm::vec3(0.0f,0.0f,1.1f);
            float dist = glm::length(camera.Position - winpos);
            std::map<int,glm::vec3> transit;
            transit[2] = winpos;
            sorted[dist] = transit;
        }
        ...
        {
            model = glm::mat4(1.0f);
            std::map<int,glm::vec3> obj = it->second;
            auto objit = obj.begin();
            model = glm::translate(model, objit->second);
            shader.setMat4("model", model);
            if(objit->first == 1){
        ...
        screenShader.use();
        glBindVertexArray(quadVAO);
        glBindTexture(GL_TEXTURE_2D, allviewtextureColorbuffer);	// use the color attachment texture as the texture of the quad plane
        glDrawArrays(GL_TRIANGLES, 0, 6);
